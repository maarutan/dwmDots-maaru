...
delete_duplicate_dirs() {
    ...
    # Собираем все директории, исключая .git
    dirs=$(find "$TARGET_DIR" -type d -not -path '*/.git/*')
    
    # Создаем ассоциативный массив для хранения путей директорий с одинаковыми именами
    declare -A dir_map
    ...
    while read -r dir; do
        ...
        # Если директория с таким именем уже есть, добавляем в массив
        if [[ -v "dir_map[$basename]" ]]; then
            dir_map["$basename"]+="$dir "
        else
            dir_map["$basename"]="$dir "
        fi
    done <<< "$dirs"
    ...
    for basename in "${!dir_map[@]}"; do
        dirs_list=(${dir_map[$basename]})
        
        # Если есть несколько директорий с одинаковым именем, удаляем лишние
        if [ "${#dirs_list[@]}" -gt 1 ]; then
            echo "Найдены дубликаты: $basename"
            for dir in "${dirs_list[@]:1}"; do
                echo "Удаляем $dir"
                rm -rf "$dir"
            done
        fi
